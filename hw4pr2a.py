...
def NLL(X, y, W, reg=0.0):
	...
	mu = sigmoid(X @ W)
	expression = np.multiply(y, np.log(mu)) + np.multiply((1-y), np.log(1-mu))
	nll = -sum(expression) + reg / 2 * np.linalg.norm(W) ** 2
	return nll.item(0)
...
def get_description(X, y, W):
	...
	total_p, total_r = 0, 0
	for i in range(m):
		actual, pred = y.item(i), labels.item(i)
		if actual == pred:
			count_a += 1
		if actual == 1:
			total_r += 1
...
def plot_description(X_train, y_train, X_test, y_test):
	...
	optimal_lambda = np.argmax(a_list)
	reg_opt = reg_list[optimal_lambda]
	return reg_opt
