...
def find_RMSE(W, X, y):
	...
	predicted_y = predict(W, X)
	error = y - predicted_y
	n = X.shape[0]
	MSE = np.linalg.norm(error, 2) ** 2 / n
	return np.sqrt(MSE)
...
def linreg_no_bias(X, y, reg=0.0):
	...
	n = X.shape[0]
	ones = np.eye(n)
	A = X.T @ (np.eye(n) - np.ones(n) / n)
	W_opt = np.linalg.solve(A @ X + reg * np.eye(A.shape[0]), A @ y)
	b_opt = sum((y - X @ W_opt)) / n
